simFun <- function(n=100, d=10, k=NULL, tau=.5, distribution="normal", num_sim = 250, filename = "res", clus = NULL){
  ...
  sim.grid <- createGrid(n, d, tau, distribution, num_sim)
  ...
  source("functions/jackknifeVariance.R")
  clusterExport(clus, varlist=c("sim.grid",
                                "generateData",
                                "testEquiRank",
                                "jackknifeVariance"),envir = environment())
  ...
  res <- rbindlist(parLapply(clus, sample(nrow(sim.grid)), function(r){
    ...
    X <- generateData(n,d,tau,distribution)
    test <- testEquiRank(X,500,500)
    
    res.grid <- sim.grid[rep(r,2),]
