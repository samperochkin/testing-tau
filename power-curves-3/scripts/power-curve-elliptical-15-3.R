...
large.n <- 25000 # for asymptotic results..
M <- 20000
epsilon <- seq(0,7.5,.1)
...
if(!file.exists(paste0("sim/sim-main/powerCurves3/pre-computed/",run.id,"/sigma_list.rds"))){
  ...
  saveRDS(sigma.list, paste0("sim/sim-main/powerCurves3/pre-computed/",run.id,"/sigma_list.rds"))
}else{
  sigma.list <- readRDS(paste0("sim/sim-main/powerCurves3/pre-computed/",run.id,"/sigma_list.rds"))
}
...
if(!file.exists(paste0("sim/sim-main/powerCurves3/pre-computed/",run.id,"/zeta1_list.rds"))){
  
  source("power-curves-2/functions/gFun.R")
  source("power-curves-2/functions/scoreFunction.R")
  source("power-curves-2/functions/computeZeta1.R")
  
  zeta1.list <- list()
  for(i in zeta1.grid$zeta1_id) zeta1.list[[i]] <- computeZeta1(zeta1.grid[i,], N=30000) # it calls mclapply inside (for hacs)
  saveRDS(zeta1.list, paste0("sim/sim-main/powerCurves3/pre-computed/",run.id,"/zeta1_list.rds"))
  
}else{
  zeta1.list <- readRDS(paste0("sim/sim-main/powerCurves3/pre-computed/",run.id,"/zeta1_list.rds"))
}
...
mclapply(1:nrow(full.grid), function(i){
  ...
  fwrite(res.grid, paste0("sim/sim-main/powerCurves3/results/res_grid_",run.id,"_",i,".csv"))
  NULL
}, mc.cores = mc_cores)


# res.grid <- rbindlist(lapply(list.files("sim/sim-main/powerCurves3/results",full.names = T), fread))
