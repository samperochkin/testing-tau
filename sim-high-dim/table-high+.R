...
dt <- rbindlist(list(dt,fread("dt_high_dim_3_1.csv")))
dt <- rbindlist(list(dt,fread("dt_high_dim_3_2.csv")))
...
dt.sub2 <- matrix(NA, nrow(dt.sub)+4, ncol(dt.sub)+3)
dt.sub2[-c(4,8,12,16), -c(3,7,11)] <- as.matrix(dt.sub)
...
dt.sub2[-seq(4,32,4), -c(4,8,12,16)] <- as.matrix(dt.sub)
