...
if(!file.exists(paste0("sim/sim-main/powerCurves2/pre-computed/",run.id,"/sigma_list.rds"))){
  ...
  saveRDS(sigma.list, paste0("sim/sim-main/powerCurves2/pre-computed/",run.id,"/sigma_list.rds"))
}else{
  sigma.list <- readRDS(paste0("sim/sim-main/powerCurves2/pre-computed/",run.id,"/sigma_list.rds"))
}
...
if(!file.exists(paste0("sim/sim-main/powerCurves2/pre-computed/",run.id,"/zeta1_list.rds"))){
  ...
  saveRDS(zeta1.list, paste0("sim/sim-main/powerCurves2/pre-computed/",run.id,"/zeta1_list.rds"))
  
}else{
  zeta1.list <- readRDS(paste0("sim/sim-main/powerCurves2/pre-computed/",run.id,"/zeta1_list.rds"))
}
...
mclapply(1:nrow(full.grid), function(i){
  ...
  fwrite(res.grid, paste0("sim/sim-main/powerCurves2/results/res_grid_",run.id,"_",i,".csv"))
  NULL
}, mc.cores = mc_cores)


# res.grid <- rbindlist(lapply(list.files("sim/sim-main/powerCurves2/results",full.names = T), fread))
