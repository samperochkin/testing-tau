...
dt2 <- dcast(dt[d == 10 & dtau == 0 & test_type %in% tt & n %in% c(100,200,400),], test_type~tau+n, value.var = "m_decision")
print(xtable(dt2, digits = 3), include.rownames=FALSE)

cols <- names(dt2)[-c(1)]
dt2[ , (cols) := lapply(.SD, round, digits=3), .SDcols = cols]

dt3 <- matrix(NA, nrow(dt2)+1, ncol(dt2)+3)
dt3[-1,-c(2,6,10)] <- as.matrix(dt2)
colnames(dt3) <- rep(" ",ncol(dt3))
colnames(dt3)[-c(2,6,10)] <- colnames(dt2)
...
dt2 <- dcast(dt[d == 10 & tau == .3 & dtau == .1 & test_type %in% tt,], test_type~n+dtau_type, value.var = "m_decision")
print(xtable(dt2, digits = 3), include.rownames=FALSE)

cols <- names(dt2)[-c(1)]
dt2[ , (cols) := lapply(.SD, round, digits=3), .SDcols = cols]

dt3 <- matrix(NA, nrow(dt2), ncol(dt2)+4)
dt3[,-c(2,5,8,11)] <- as.matrix(dt2)
colnames(dt3) <- rep(" ",ncol(dt3))
colnames(dt3)[-c(2,5,8,11)] <- colnames(dt2)
