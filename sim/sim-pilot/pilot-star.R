...
# Levels are not the same (since bootstrap is for H0 genenral),
# Power is better with Monte Carlo

source("sim-pilot/functions/generateData.R")
X <- generateData(n=50, d=50, tau=.5, dtau=.15, dtau_type = "column", distribution = "normal")
M <- 10000
...
# eig <- eigen(S)
# 
# MC.euc.I <- c(eig$val %*% matrix(rchisq(M*p, 1),p,M))
...
#### Boot
Boot <- replicate(M, {
  zz <- sqrt(n)*((Ths-matrix(Tbs,p,n,byrow=T))/(n*(n-1)) - t.cen/n) %*% rnorm(n)
