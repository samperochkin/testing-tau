...
fun <- function(tau2, tau1 = tau1.hat){
  
  theta <- c(tau2theta(tau1,3), tau2theta(tau2,3))
  scoreFunctionTOP(X, family = "clayton", params = list(theta=theta))
  # ####
  # # In fun setup
  # 
  # n <- nrow(X)
...
X <- rHAC(n, hhac)
pcaPP::cor.fk(X)
tau1.hat <- pcaPP::cor.fk(X[,I1])
tau1.hat <- mean(tau1.hat[upper.tri(tau1.hat)])

fun(tau+dtau, tau)

# ths <- (tau+dtau) + seq(-.2,.2,.01)
taus <- (tau+dtau) + seq(-.01,.01,.01)
ll <- sapply(taus, fun, tau1=tau)

# plot taus
plot(taus, ll, type = "l")
points(taus, ll)
abline(h=0, lty=2, col=2)
abline(v=taus[which.min(abs(ll))], lty=2, col=2) # estimated value
abline(v=tau+dtau, lty=3, col=3) # true value
I2 <- 1:2
tau.hat <- pcaPP::cor.fk(X[,I2])
tau.hat <- mean(tau.hat[upper.tri(tau.hat)])
abline(v=tau.hat, lty=3, col=4) # inverse tau.hat

# plot theta
ths <- tau2theta(taus,3)
plot(ths, ll, type = "l")
points(ths, ll)
abline(h=0, lty=2, col=2)
abline(v=ths[which.min(abs(ll))], lty=2, col=2) # estimated value
abline(v=tau2theta(tau+dtau,3), lty=3, col=3) # true value
I2 <- 1:2
abline(v=tau2theta(tau.hat,3), lty=3, col=4) # inverse tau.hat

theta2tau(ths[which.min(abs(ll))],3); tau + dtau
ths[which.min(abs(ll))]; tau2theta(tau + dtau, 3)
