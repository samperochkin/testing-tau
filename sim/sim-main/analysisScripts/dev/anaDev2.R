...
dt <- rbindlist(c(list(dt),lapply(fii[4:5],fread)))
...
xdt <- dcast(dt2[norm == "Euclidean" & S == "Sh" & d %in% c(5,15) &
                   dtau == 0 & tau %in% c(0,.3,.6) & Sh %in% c("ShP","ShJ")],
             formula = Sh + d ~ tau + n,
             value.var = "level")
xdt
...
xdt <- dcast(dt2[norm == "Euclidean" & S == "I" &
                   ...
                   dtau == 0 & tau %in% c(0,.3,.6) & Sh %in% c("ShJ")],
             formula = d ~ tau + n,
             value.var = "NN")
xdt
...
xdt <- dcast(dt3[norm == "Euclidean" & S == "Sh" & isShPd == T &
                   n %in% c(50,100,150) & d %in% c(5,15,25,50,100) &
                   dtau == 0 & tau %in% c(0,.3,.6) & Sh %in% c("SbP","SbJ")],
             formula = Sh + d ~ tau + n,
             value.var = "N")
xdt
xdt <- dcast(dt2[norm == "Euclidean" & S == "Sh" &
                   n %in% c(50,100,150) & d %in% c(5,15,25,50,100) &
                   dtau == 0 & tau %in% c(0,.3,.6) & Sh %in% c("SbP","SbJ")],
             formula = Sh + d ~ tau + n,
             value.var = "level")
...
xdt <- dcast(dt2[norm == "Euclidean" & S == "Sh" &
             ...
             value.var = "N")
xdt
any(is.na(dt[norm == "Euclidean" & S == "Sh" & n %in% c(50) & d %in% c(5,15,25,50,100) &
      dtau == 0 & tau %in% c(0) & Sh %in% c("SbP","SbJ")]$pvalue))
