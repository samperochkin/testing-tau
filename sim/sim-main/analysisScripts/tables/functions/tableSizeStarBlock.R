tableSizeStarBlock <- function(R, M, distribution){
  ...
  cat("\\begin{table}
\\caption{Estimated sizes (in \\%) for the tests of $H_0^*$ with $\\mathcal{G}$ of \\eqref{eq:G-blocks}, performed at the nominal level 5\\%.
      Each entry is based on $",M,"$ $n\\times d$ datasets from a",distribution_text,"with Kendall's tau matrix $\\T$ as in Eq.~\\eqref{eq:T-block} with $c_{k\\ell} = 0.4 - (0.15)|k-\\ell|$ for all $k,\\ell \\in \\{1,\\dots,3\\}$.}
      ",paste0("\\label{tab:sim-level-star-blocks-",distribution,"}"),"
      \\begin{center}
      \\scriptsize
      \\vskip-9pt
      \\begin{tabular}{*{2}{l}*{3}{r}@{\\hspace{0.7cm}}*{3}{r}*{1}{c}*{3}{r}@{\\hspace{0.7cm}}*{3}{r}}
      \\toprule
      & &  \\multicolumn{6}{c}{$E_{np}$ with $\\SA = \\Sh_{np}$} & {\\hspace{0.2cm}} & \\multicolumn{6}{c}{$M_{np}$ with $\\SA = \\Sh_{np}$} \\\\
      ...
      &18&", rowPart(), "\\\\ 
      \\midrule
      & &  \\multicolumn{6}{c}{$E_{np}$ with $\\SA = (1/n)\\I_{p}$} & {\\hspace{0.2cm}} & \\multicolumn{6}{c}{$M_{np}$ with $\\SA = (1/n)\\I_{p}$} \\\\
      \\cmidrule(lr){3-8}\\cmidrule(lr){10-15}
      &&  \\multicolumn{3}{c}{balanced} & \\multicolumn{3}{c}{unbalanced} & &  \\multicolumn{3}{c}{balanced} & \\multicolumn{3}{c}{unbalanced}\\\\
      \\cmidrule(lr){3-8}\\cmidrule(lr){10-15}
      $\\Sh_{np}$ & $d$\\big|$n$ & 50 & 150 & 250 & 50 & 150 & 250 && 50 & 150 & 250 & 50 & 150 & 250 \\\\
