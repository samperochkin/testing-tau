tablePowerBlock <- function(R, M, distribution, dtau){
  ...
  cat("\\begin{table}
      ...
      \\caption{Estimated rejection rates of tests of $H_0$ with $\\B$ the block membership associated to $\\mathcal{G}$ of \\eqref{eq:G-blocks} and $\\SA = (1/n)\\I_{p}$, performed at nominal level $5$\\%.
      Each entry is based on $",M,"$ $n\\times d$ datasets from a ",distribution_text," with Kendall's tau matrix $\\bs{T}_\\Delta$ in Eq.~\\eqref{eq:departure} (i) with $\\Delta = ",dtau,"$ and $\\T$ as in Eq.~\\eqref{eq:T-block} with $c_{k\\ell} = 0.4 - (0.15)|k-\\ell|$ for all $k,\\ell \\in \\{1,2,3\\}$.}
      ",paste0("\\label{tab:sim-power-blocks-",distribution,"}"),"
      \\begin{center}
      \\scriptsize
      \\vskip-9pt
      \\begin{tabular}{*{2}{l}*{3}{r}@{\\hspace{0.7cm}}*{3}{r}*{1}{c}*{3}{r}@{\\hspace{0.7cm}}*{3}{r}}
      \\toprule
      & &  \\multicolumn{6}{c}{$E_{np}$ with $\\SA=(1/n)\\I_p$ for single dep.} & {\\hspace{0.2cm}} & \\multicolumn{6}{c}{$M_{np}$ with $\\SA=(1/n)\\I_p$ for single dep.} \\\\
