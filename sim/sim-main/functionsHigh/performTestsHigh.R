performTestsHigh <- function(X, M){
  ...
  if(sigma.hat[2]-sigma.hat[1] > 0){
    ...
    resTable[6,]$pvalue <- mean(apply(abs(Z.hat - Z.star)/sqrt(delta.hat[3]), 2, max) > supS_p)
    resTable[7,]$pvalue <- mean(apply(abs(t(Z.star) - Z.bar)/sqrt(delta.hat[2]), 1, max) > supS_d)
