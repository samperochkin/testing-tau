get_nodes_attr(dend,"type")
dend <- dendrapply(dend, function(node){
  if(identical(attr(node,"type"),"hétérogène")) attr(node,"type") <- 0
  return(node)  
})
...
dend <- dendrapply(dend, function(node){
  if(identical(attr(node,"type"),1)) attr(node,"type") <- "homogène"
  return(node)  
})
...
pdf(file = "arbre3-legend.pdf", width = 7.5, height = 5)
gg
dev.off()
...
pdf(file = "arbre3.pdf", width = 5, height = 5)
gg
dev.off()
