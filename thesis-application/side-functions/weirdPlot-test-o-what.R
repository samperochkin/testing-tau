...
weirdPlot <- function(dend,non.leaf){
  ...
  type <- sapply(vec.address2, function(v){
    ...
    if(identical(attr(node, "valid"),TRUE)) return("homogène")
    # attr(node, "type")
    return("indéfini")
  })
  ...
  al <- sapply(vec.address2, function(v){
    node <- getSubDend(dend,v)
    if(is.leaf(node)) return(NA)
    if(identical(attr(node, "valid"),FALSE)) return(.5)
    if(identical(attr(node, "valid"),TRUE) & identical(attr(node, "type"),0)) return(1)
    if(identical(attr(node, "valid"),TRUE)) return(.5)
    # attr(node, "type")
    return(.5)
  })
  ...
  gg <- ggraph(mygraph, layout = 'dendrogram', circular = F) + 
    ...
    geom_node_point(aes(x = x, y=y, colour=type, filter = !leaf, alpha = factor(al, levels=c(.5,1))), size = 5) +
    theme_void() +
    theme(legend.text=element_text(size=15),
          legend.title=element_text(size=15)) +
    # theme(legend.position = c(.18,.87)) +
    scale_alpha_discrete(limits = c(.5,1)) +
