...
simFunLowBlock2 <- function(n=100, d=6, design="balanced", dtau=0, distribution="normal", num_sim = 250, filename = "resLow", cl = NULL){
  
  # packages
  library(parallel)
  library(mvtnorm)
  library(data.table)
  sapply(list.files("functionsLowBlock2/", full.names = T), source, local = environment())
