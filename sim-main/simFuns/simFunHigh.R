...
simFunHigh <- function(n=100, d=10, tau=.5, dtau=0, distribution="normal", num_sim = 250, filename = "resLow", clus = NULL){
  
  # packages
  library(parallel)
  library(mvtnorm)
  library(data.table)
  sapply(list.files("functionsHigh/", full.names = T), source, local = environment())
  ...
  # res <- rbindlist(lapply(1:nrow(sim.grid), function(r){
    ...
    cbind(sim.grid[r,],performTestsHigh(X,M=M))
  }))
