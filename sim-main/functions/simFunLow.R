...
simFun <- function(n=100, d=10, k=NULL, tau=.5, dtau=0, distribution="normal", num_sim = 250, filename = "res", clus = NULL){
  ...
  # res <- rbindlist(lapply(1:nrow(sim.grid), function(r){
    ...
    res.grid[(1+length(test.plugin)):(length(test.plugin)+length(test.jackknife)), `:=` (sigma = c(rep("jackknife",8),"bootstrap"), test = names(test.jackknife), pval = test.jackknife)]
