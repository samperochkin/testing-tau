library(mvtnorm)
sapply(list.files("sim-main/functionsHigh2/", full.names = T), source, local=environment())
X <- generateData(n=250, d=15, tau=0, dtau=0, dtau_type = "none", distribution = "normal")
performTestsHigh2(X,5000)

res <- do.call("rbind",lapply(1:500,function(dummy){
  X <- generateData(n=100, d=20, tau=.25, dtau=0, dtau_type = "none", distribution = "normal")
  n <- nrow(X)
  d <- ncol(X)
  p <- d*(d-1)/2
