...
simFunLow <- function(n=100, d=10, tau=.5, dtau=0, distribution="normal", num_sim = 250, filename = "resLow", clus = NULL){
  ...
  #### WHY ISNT SIMGRID PASSED -- NO
  clusterExport(clus,
                varlist=c("sim.grid",Filter(function(x) inherits(get(x), "function"), ls())),
                envir = environment())
  
  # simuls
  res <- parLapply(clus, sample(nrow(sim.grid)), function(r){
  # res <- rbindlist(lapply(1:nrow(sim.grid), function(r){
    ...
    cbind(sim.grid[r,],performTestsLow(X,M=M))
  })
